...
//data obtaining
$up_backend4_passValue = $_POST['up_backend4_passValue']; //shop item ID
//echo "<script>alert('{$up_backend4_passValue}');</script>";
require_once 'blobStorageCon.php'; //blob storage connection
require_once 'sqlConnection.php'; //sql connection 
//query setup
$additional_query = "SELECT * FROM additional WHERE shopItemID = $up_backend4_passValue";
$additional_query_stmt = sqlsrv_query($conn,$additional_query);
if($additional_query_stmt == false){
   // echo "<script>alert('Operation could not be performed : ErrorUP4QUERY');
    //window.location.href = 'updateItems.php';</script>";
}else{
    $additionalInfoObj = array("id"=>"","Description"=>"");
    while($row = sqlsrv_fetch_array($additional_query_stmt,SQLSRV_FETCH_ASSOC)){
        $additionalInfoObj["Description"] = $row['addInfo'];
        $additionalInfoObj["id"] = $row['addID'];
    }
}
$string = $additionalInfoObj["Description"];
$string2 = "";
$length = 0;
...
if($length > 70){
  ...
  $string2 = $additionalInfoObj["Description"];
}
?>
<!DOCTYPE html>
<html lang="en">
...
.custom-input {
            ...
            padding: 8px;             /* Inner padding for the input */
            font-size: 16px;          /* Font size */
        }
        .custom-label {
            font-weight: 600;         /* Semi-bold font for the label */
        }
/*form end (inputs)*/
.custom-file-upload {
    position: relative;
...
?>
            ...
            <!--ORG INFO POP UP BOX START FIXME: CHANGE 5(VALIDATED THE INPUTS AND THE VALUES )--->
  ...
  <form class="form" action="orgInfoUpdate.php" method="post">
    <div class="form-group">
      ...
      <input
        ...
        name="orgName"
        id="orgName"
        type="text" maxlength="160"/><!--FIXME: CHANGE 7-->
        ...
        name="mail"
        id="mail"
        type="email"/><!--FIXME: CHANGE 6(THE TYPE)-->
                </div>
            </div>
        </div>
        ...
        name="phone"
        id="phone"
        type="number" min="0" max="15"/> <!--FIXME: CHENGE 7-->
                </div>
                <div class="col-md-6" style="padding:0px;">
                <input
        required
...
<script>
  ...
  <div class="content-wrapper">
    ...
    <div class="container">
      ...
      <div class="row">
        <div class="col-md-12">
          <label for="inputName2" class="form-label custom-label">Additional Info</label>
        </div>
      </div>
      <form action="updateBackend4phpcodes.php" method="post">
      <div class="row">
        <div class="col-md-12">
          <div style="display: flex;">
            <input hidden name="temp_data_itemID" type="text" value="<?php echo "{$up_backend4_passValue}";?>">
            <input name="additional_Info" type="text" class="form-control custom-input" style="margin-left: 5px;" value="<?php echo "{$string2} . . . . . . . . . .";?>">
            <button class="custom-btn btn-1" name="additionalInBtn" style="margin-left: 10px;">Add</button>
          </div>
        </div>
      </div>
      </form>
      <br>
      <form action="confirmation2UpdateFinish.php" method="post" enctype="multipart/form-data">
      <!-- Manuals and Side Images Section -->
      <div class="row">
        <div class="col-md-2">
          <label for="inputName2" class="form-label custom-label">Manuals</label>
        </div>
        <div class="col-md-2" style="margin-left: 8px;">
          <label for="inputName2" class="form-label custom-label">Side Images</label>
        </div>
      </div>
        ...
        <div class="col-md-12">
          ...
          <div style="display: flex;">
            ...
            <label for="input-file2" id="drop-area2">
              <input type="file" accept="application/pdf" name="manualInput" id="input-file2" hidden>
              <div id="img-view2" class="testClass" style="display: flex; align-items: center; justify-content: center; height: 100px; border: 1px dashed #ccc;">
                <p style="text-align: center; margin: 0;" id="fileNameSection">Drag & drop or click here</p>
              </div>
            </label>

            <!-- Side Images Upload Loop -->
            <?php
            $inputNamings = array("a", "b", "c", "d");
            for ($count4IMG = 0; $count4IMG < 4; $count4IMG++) {
              echo '
              <label id="drop-area_' . $count4IMG . '">
                <input type="file" accept="image/*" name="smallImageInput_' . $inputNamings[$count4IMG] . '" id="input-file_' . $count4IMG . '" hidden>
                <div id="img-view_' . $count4IMG . '" class="testClass" style="display: flex; align-items: center; justify-content: center; height: 100px; border: 1px dashed #ccc;">
                  <p style="text-align: center; margin: 0;" id="fileNameSection_' . $count4IMG . '">Drag & drop or click here</p>
                </div>
              </label>
              <script>
                document.getElementById("input-file_' . $count4IMG . '").addEventListener("change", function() {
                  let imgLink = URL.createObjectURL(this.files[0]);
                  let imgView = document.getElementById("img-view_' . $count4IMG . '");
                  imgView.style.backgroundImage = `url(${imgLink})`;
                  imgView.textContent = "";
                  imgView.style.border = "none";
                });

                document.getElementById("drop-area_' . $count4IMG . '").addEventListener("dragover", function(e) {
                  e.preventDefault();
                });
                document.getElementById("drop-area_' . $count4IMG . '").addEventListener("drop", function(e) {
                  e.preventDefault();
                  let inputFile = document.getElementById("input-file_' . $count4IMG . '");
                  inputFile.files = e.dataTransfer.files;
                  let imgLink = URL.createObjectURL(inputFile.files[0]);
                  let imgView = document.getElementById("img-view_' . $count4IMG . '");
                  imgView.style.backgroundImage = `url(${imgLink})`;
      ...
      <!-- File Upload JS -->
      <script>
        document.getElementById('input-file2').addEventListener('change', function(event) {
          var fileInput = event.target;
          var fileName = fileInput.files[0].name;
          document.getElementById('fileNameSection').textContent = fileName;
        });

        document.getElementById("drop-area2").addEventListener("dragover", function(e) {
          e.preventDefault();
        });

        document.getElementById("drop-area2").addEventListener("drop", function(e) {
          e.preventDefault();
          let inputFile = document.getElementById("input-file2");
          inputFile.files = e.dataTransfer.files;
          let imgLink = URL.createObjectURL(inputFile.files[0]);
          let imgView = document.getElementById("img-view2");
          imgView.style.backgroundImage = `url(${imgLink})`;
          imgView.textContent = "";
          imgView.style.border = "none";
        });
      ...
      <div class="row">
        <div class="col-md-12">
        <button class="nextButton" name="endingButton" value="<?php echo "{$up_backend4_passValue}";?>" style="color: white;">Finish</button>
        </form>
        </div>
      </div>
    </div>
