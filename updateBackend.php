...
//selecting query start
$itemQuery = "SELECT *FROM shopItems WHERE shopItemID = $updateID";
$selectResultItemstb = sqlsrv_query($conn,$itemQuery);
if($selectResultItemstb == false){
    die(print_r(sqlsrv_errors(),true));
}
else{
    $selectedDetails = array("itemName"=>"","price"=>"","longName"=>"","des1"=>"","des2"=>"","brandIconName"=>"","address"=>"","type"=>"","productLink"=>"","imgName" => "");
    while($row = sqlsrv_fetch_array($selectResultItemstb,SQLSRV_FETCH_ASSOC)){
        $selectedDetails['itemName'] = $row['itemName'];
        $selectedDetails['price'] = $row['price'];
        $selectedDetails['imgName'] = $row['imgName'];
        $selectedDetails['longName'] = $row['longName'];
        $selectedDetails['des1'] = $row['des1'];
        $selectedDetails['des2'] = $row['des2'];
        $selectedDetails['brandIconName'] = $row['brandIconName'];
        $selectedDetails['address'] = $row['address'];
        $selectedDetails['type'] = $row['type'];
        $selectedDetails['productLink'] = $row['productLink'];
    }
}
//selecting query end
?>
...
<script>
  ...
  <div class="container">
    ...
    <div class="row" style="margin-bottom:10px;">
      <div class="col-md-12">
        <div style="display:flex;"><h4>Update an item</h4></div>
      </div>
    </div>
    <form action="updateBackend2.php" method="post" enctype="multipart/form-data">
      <div class="row">
        <?php 
        $arr1 = array("Item Name", "Price", "Long Name", "Description 1", "Description 2");
        $arr2 = array("Brand Name", "Address", "Type", "Product Link");
        $arrone = array($selectedDetails['itemName'], $selectedDetails['price'], $selectedDetails['longName'], $selectedDetails['des1'], $selectedDetails['des2']);
        $arrtwo = array($selectedDetails['brandIconName'], $selectedDetails['address'], $selectedDetails['type'], $selectedDetails['productLink']);
        $arr3 = array("Item_Name", "Price", "Long_Name", "Description_1", "Description_2");
        $arr3WordLimit = array(60,0,180,660,200);
        $wordLimitCount3 = 0;
        $arr4 = array("Brand_Name", "Address", "Type", "Product_Link");
        $arr4WordLimit = array(50,130,0,0);

        for ($count4 = 0; $count4 < 5; $count4++) {
          if($arr1[$count4] == "Price" && $arr3[$count4] == "Price"){
            echo "
            <div class=\"col-md-6\">
              <label for=\"inputName\" class=\"form-label custom-label\">$arr1[$count4]</label>
              <input type=\"number\" min=\"0\" class=\"form-control custom-input\" id=\"inputName\" name='$arr3[$count4]' value=\"$arrone[$count4]\">
            </div>
            <script>
            document.getElementById('inputName').addEventListener('change', function(event) {
              var inputValueCheck = document.getElementById('inputName').value;
            ...
            echo "
          <div class=\"col-md-6\">
            <label for=\"inputName\" class=\"form-label custom-label\">$arr1[$count4] ($arr3WordLimit[$count4]/<span id=\"currentNumber-{$arr3[$count4]}\"></span>)</label>
            <input type=\"text\" class=\"form-control custom-input\" maxlength = \"$arr3WordLimit[$count4]\" id=\"inputName-{$arr3[$count4]}\" name='$arr3[$count4]' onInput=\"fu1(event)\" value=\"$arrone[$count4]\">
          </div>
          <script>
          document.addEventListener('DOMContentLoaded',function(){
          let currentCharBeforeEnter = document.getElementById('inputName-{$arr3[$count4]}').value;
               let lengthBeforeEnter = currentCharBeforeEnter.length;
               document.getElementById('currentNumber-{$arr3[$count4]}').innerText = lengthBeforeEnter;
          });
               

          document.getElementById('inputName-{$arr3[$count4]}').addEventListener('input', function(event) {
               let currentChar = document.getElementById('inputName-{$arr3[$count4]}').value;
               let length = currentChar.length;
                 document.getElementById('currentNumber-{$arr3[$count4]}').innerText = length;
               });
          </script>
          ";
          }
          if ($count4 != 4) {
            if($arr2[$count4] == "Type" && $arr4[$count4] == "Type"){
              echo "
              <div class=\"col-md-6\">
              <label for=\"inputName2\" class=\"form-label custom-label\">$arr2[$count4]</label>
              <select class=\"form-control custom-input\" id=\"inputName2\" name='$arr4[$count4]' value=\"$arrtwo[$count4]\">
            ...
            if($arr2[$count4] == "Product Link" && $arr4[$count4] == "Product_Link"){
              echo "
              <div class=\"col-md-6\">
              <label for=\"inputName2\" class=\"form-label custom-label\">$arr2[$count4]($arr4WordLimit[$count4]/<span id=\"currentNumber-{$arr4[$count4]}\">0</span>)</label>
              <input type=\"text\" class=\"form-control custom-input\" id=\"inputName2-{$arr4[$count4]}\" name='$arr4[$count4]' value=\"$arrtwo[$count4]\">
          </div>
           <script>
           document.addEventListener('DOMContentLoaded',function(){
          let currentCharBeforeEnter = document.getElementById('inputName2-{$arr4[$count4]}').value;
               let lengthBeforeEnter = currentCharBeforeEnter.length;
               document.getElementById('currentNumber-{$arr4[$count4]}').innerText = lengthBeforeEnter;
          });
        document.getElementById('inputName2-{$arr4[$count4]}').addEventListener('input', function(event) {
     let currentChar = document.getElementById('inputName2-{$arr4[$count4]}').value;
     let length = currentChar.length;
       document.getElementById('currentNumber-{$arr4[$count4]}').innerText = length;
     });
      </script>
              ";
          }else{
            echo "
            <div class=\"col-md-6\">
              <label for=\"inputName2\" class=\"form-label custom-label\">$arr2[$count4]($arr4WordLimit[$count4]/<span id=\"currentNumber-{$arr4[$count4]}\"></span>)</label>
              <input type=\"text\" class=\"form-control custom-input\" maxlength = \"$arr4WordLimit[$count4]\" id=\"inputName2-{$arr4[$count4]}\" name='$arr4[$count4]' value=\"$arrtwo[$count4]\">
            </div>
            <script>
          document.addEventListener('DOMContentLoaded',function(){
          let currentCharBeforeEnter = document.getElementById('inputName2-{$arr4[$count4]}').value;
        ...
        ?>
        <div class="col-md-6">
          <label for="inputName2" class="form-label custom-label">Main Image</label>
          <div class="custom-file-upload">
            <input type="file" accept=".jpg" class="form-control custom-input" id="productImage" name="productImage" placeholder="image">
            <span style="background-color:#212529;" class="upload-button" id="selectedImageName">Click here to choose File</span>
            <script>
            
            document.getElementById('productImage').addEventListener('change', function(event) {
     const file = event.target.files[0];
     if (file) {
         var test1 = document.getElementById('productImage').value;
         document.getElementById('selectedImageName').innerText = test1;
     }
 });     
         </script>
          </div>
          <br>
          <!-- Checkbox UI Start -->
          <div class="checkbox-wrapper-4" style="margin-top:8px;">
            <input class="inp-cbx" id="morning" name="morning" type="checkbox" onClick="click4()">
            <label class="cbx" for="morning">
              <span style="border-color:#007bff;">
                <svg width="12px" height="10px"></svg>
              </span>
              <span style="color:black;">Change the image</span>
            </label>
          ...
          <!-- Checkbox UI End -->
          <script>
            var sample = document.getElementById('morning');
            if (sample.checked) {
              document.getElementById('productImage').disabled = false;
              document.getElementById('selectedImageName').style.backgroundColor = '#023e8a';

            } else {
              document.getElementById('productImage').disabled = true;
              document.getElementById('selectedImageName').style.backgroundColor = '#495057';
            }

            function click4() {
              var sample1 = document.getElementById('morning');
              if (sample1.checked) {
                document.getElementById('productImage').disabled = false;
                document.getElementById('selectedImageName').style.backgroundColor = '#023e8a';
              } else {
          ...
          <input hidden type="text" value="<?php echo $updateID; ?>" name="idinput">
        </div>
      </div>
      <br>
      <div class="row">
