...
<!--
      ...
      <div class="container my-4" style="background-color:#e5e5e5;">
    <h2 class=" mb-4">Remove items</h2>
    <form action="deleteBackend.php" method="post" onsubmit="return confirmDeletion();">
    ...
    <script>
      ...
      <table class="table table-bordered table-hover mb-0">
        ...
        <tbody id="table-body">
          ...
          $allItems = array("shopItemID"=>"","itemName"=>"","price"=>"","longName"=>"","des1"=>"","des2"=>"","brandIconName"=>"","address"=>"","type"=>"");
          while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
              $allItems['shopItemID'] = $row['shopItemID'];
              $allItems['itemName'] = $row['itemName'];
              $allItems['price'] = $row['price'];
              $allItems['longName'] = $row['longName'];
              $allItems['des1'] = $row['des1'];
              $allItems['des2'] = $row['des2'];
              $allItems['brandIconName'] = $row['brandIconName'];
              $allItems['address'] = $row['address'];
              $allItems['type'] = $row['type'];

          //}
?>
        <tr>
            <td><?php echo "{$allItems['shopItemID']}";?></td>
            <td><?php echo "{$allItems['itemName']}";?></td>
            <td><?php echo "{$allItems['price']}";?></td>
            <td><?php echo "{$allItems['brandIconName']}";?></td>
            <td><?php echo "{$allItems['address']}";?></td>
            <td><?php echo "{$allItems['type']}";?></td>
            
            <td>
              <button name="deleteID" type="submit" id="deleteID" value="<?php echo "{$allItems['shopItemID']}";?>" class="btn btn-delete btn-sm">
                Delete
              </button>
            </td>
          </tr>
