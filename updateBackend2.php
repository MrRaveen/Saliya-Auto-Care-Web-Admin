...
if($Item_Name == "" || $Price == "" || $Long_Name == "" || $Description_1 == "" || $Description_2 == "" || $Brand_Name == "" 
|| $Address == "" || $Type = "" || $Product_Link == ""){

    echo "<script>alert('Please enter the required data to proceed');
    window.location.href = 'updateItems.php'; </script>";
}else{
    if(isset($_POST['morning'])){
        $newFile = $_FILES['productImage'];
        $selectQuery = "SELECT imgName FROM shopItems WHERE shopItemID = $sub2btn";
        $selectstmt = sqlsrv_query($conn,$selectQuery);
        if($selectstmt == false){
            //CODE:100E
            echo "<script>alert('Operation could not be performed (CODE:100E)');
            window.location.href = 'updateItems.php';</script>";
        }else{
        /////////////////////////////////
        //getting the image name
        $imageName = "";
        while($row2 = sqlsrv_fetch_array($selectstmt,SQLSRV_FETCH_ASSOC)){
            $imageName = $row2['imgName'];
        }
        ...
        //inserting the image with the previous name (without changing the name of the image in the database)
        $content1 = fopen($newFile['tmp_name'], "r");
        try{
            $blobClient->createBlockBlob($containerName,  $newFile['name'], $content1);
        }
        catch(ServiceException $f){
            echo "Error: " . $f->getMessage();
        }
        //sql update section start
        $type3 = $_POST['Type'];
        $allDetailsQuery = "UPDATE shopItems SET itemName = ?,price = ?,longName = ?,des1 = ?,des2 = ?,brandIconName = ?,address = ?,type = ?,productLink = ? WHERE shopItemID = ?";
        $allDetailsQuerypara = array($Item_Name,$Price,$Long_Name,$Description_1,$Description_2,$Brand_Name,$Address,$type3,$Product_Link,$sub2btn);  
        $allDetailsQueryresult = sqlsrv_query($conn,$allDetailsQuery,$allDetailsQuerypara);
        if($allDetailsQueryresult == false){
            //CODE:200E
            echo "<script>alert('Operation could not be performed (CODE:200E)');
            window.location.href = 'updateItems.php';</script>";
        }
        else{
        ...
        $type2 = $_POST['Type']; //another variable for the type
        $query = "UPDATE shopItems SET itemName = ?,price = ?,longName = ?,des1 = ?,des2 = ?,brandIconName = ?,address = ?,type = ?,productLink = ? WHERE shopItemID = ?";
        $para1 = array($Item_Name,$Price,$Long_Name,$Description_1,$Description_2,$Brand_Name,$Address,$type2,$Product_Link,$sub2btn);  
        $result = sqlsrv_query($conn,$query,$para1);
        if($result == false){
            //CODE:300E
            echo "<script>alert('Operation could not be performed (CODE:300E)');
            window.location.href = 'updateItems.php';</script>";
        }
        else{
           // echo "<script>alert('Data updated');</script>";
...
.scrollable {
            width: 1100px; /* Width of the container */
            height: 350px; /* Height of the container */
            overflow-y: auto; /* Enables vertical scrolling */
            border: 1px solid #ccc; /* Optional: adds a border */
            padding: 10px; /* Optional: adds padding inside */
...
-->
  ...
  <div class="content-wrapper">
    ...
    <div class="container">
      ...
      <div class="row">
        <div class="col-md-12">
          <label for="inputName2" class="form-label custom-label">New Specifications</label>
        </div>
      </div>
      <div class="row">
        <div class="col-md-12">
          ...
          <div class="scrollable">
            <div class="col-md-12">
              <?php 
              $tquery = "SELECT * FROM specifications WHERE shopItemID = $sub2btn";
              $sperun = sqlsrv_query($conn, $tquery);
              if ($sperun == false) {
                die(print_r(sqlsrv_errors(), true));
              } else {
                $numRows = sqlsrv_num_rows($sperun);
                $countrows = 1;
                $TypeSecond = $_POST['Type'];
                while ($row = sqlsrv_fetch_array($sperun, SQLSRV_FETCH_ASSOC)) {
                  $value1 = $row['name'];
                  $value2 = $row['description'];
                  $value3 = $row['specID'];
                  if ($countrows != $numRows) {
                    echo '
                    <form action="updateBackend2phpcodes.php" method="post">
                    <input hidden type="text" value="'.$TypeSecond.'" name="Type">
                    <input hidden type="text" value="'.$sub2btn.'" name="idinput">
                    <input hidden type="text" value="'.$Item_Name.'" name="Item_Name">
                    <input hidden type="text" value="'.$Price.'" name="Price">
                    <input hidden type="text" value="'.$Long_Name.'" name="Long_Name">
                    <input hidden type="text" value="'.$Description_1.'" name="Description_1">
                    <input hidden type="text" value="'.$Description_2.'" name="Description_2">
                    <input hidden type="text" value="'.$Brand_Name.'" name="Brand_Name">
                    <input hidden type="text" value="'.$Address.'" name="Address">
                    <input hidden type="text" value="'.$Product_Link.'" name="Product_Link">
                    <div style="display:flex;">
                      <input type="text" class="form-control custom-input" value="' . $value1 . '" name="nameInput">
                      <input type="text" class="form-control custom-input" style="margin-left:5px;" value="' . $value2 . '" name="descriptionInput">
                      <button name="specificationdatainfo" value="'.$value3.'" class="custom-btn btn-1" style="margin-left:10px; color:white;">Save</button>
                    </div>
                    <br>
                    </form>';
                  }
      ...
      <br>
      <form action="updateBackend3.php" method="post" enctype="multipart/form-data">
      <button class="nextButton" name="finishingButton" value="<?php echo "{$sub2btn}"; ?>" style="color:white;">Next</button>
      </form> 
    </div>
  </div>
